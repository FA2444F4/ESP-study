...
void wifi_cmd_handler(const char *command, const char *args, cmd_responder_t responder, void *context)
{
    ...
    // 命令: test_device_set_wifi_connect
    else if (strcmp(command, "test_device_set_wifi_connect") == 0) {
        if (args == NULL) {
        ...
        if (strcmp(args, "on") == 0) {
            ...
            if (bits & WIFI_CONNECTED_BIT) {
                responder("OK: Already connected", context);
                return;
            }
            ...
            if (bits & WIFI_CONNECTED_BIT) {
                ...
                // 创建UDP任务
                if (s_udp_task_handle == NULL) {
                    xTaskCreate(udp_discover_task, "udp_discover_task", 4096, NULL, 5, &s_udp_task_handle);
                }
            } else {
                // 失败 (超时或收到断开事件)
                responder("FAIL: Wi-Fi connection failed or timed out", context);
                esp_wifi_stop(); // 停止Wi-Fi栈以节省功耗
            }
        }
        // 参数: off
        else if (strcmp(args, "off") == 0) {
            wifi_disconnect();
            responder("OK: Wi-Fi disconnected", context);
        }
        else {
